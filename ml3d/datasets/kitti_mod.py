...
class KITTI_MOD(BaseDataset):
    ...
    def read_calib(path):
        ...
        P0 = KITTI_MOD._extend_matrix(P0)
        P1 = KITTI_MOD._extend_matrix(P1)
        P2 = KITTI_MOD._extend_matrix(P2)
        P3 = KITTI_MOD._extend_matrix(P3)
    ...
    def get_split(self, split):
        ...
        """
        return KITTIMODSplit(self, split=split)
...
class Object3d(BEVBox3D):
    ...
    def __init__(self, center, size, label, calib=None):
        ...
        class_name = label[0] if label[0] in KITTI_MOD.get_label_to_names().values(
        ) else 'DontCare'
...
DATASET._register_module(KITTI_MOD)
